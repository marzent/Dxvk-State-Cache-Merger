...
struct DxvkStateCache {
    ...
    struct Entry: Equatable, Hashable {
        ...
        var rawArray: [UInt8] {
            return [stageMask] + withUnsafeBytes(of: UInt32(data.count), Array.init).prefix(3) + sha1Hash + data
        }
    ...
    init(inputData: Data) throws {
        ...
        while dataToProcess.count > 0 {
            ...
            dataToProcess = dataToProcess.dropFirst(entries.last!.data.count + Entry.headerByteSize)
        }
    }
}
